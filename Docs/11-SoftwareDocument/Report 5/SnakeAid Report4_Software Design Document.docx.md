...
interface IEmergencyConsultationService {
  +CreateEmergencyRequestAsync(requesterId, request)
  +AcceptEmergencyRequestAsync(requestId, expertId)
  +RejectEmergencyRequestAsync(requestId, expertId)
}
...
interface IConsultationPaymentService {
  +PayScheduledBookingAsync(userId, bookingId, request, cancellationToken)
  +PayEmergencyRequestAsync(userId, requestId, request, cancellationToken)
  +ConfirmConsultationPaymentAsync(transactionId, cancellationToken)
  +ConfirmConsultationPaymentByOrderCodeAsync(orderCode, cancellationToken)
...
ConsultationInstantController ..> IEmergencyConsultationService
ConsultationPaymentsController ..> IConsultationPaymentService
ConsultationsController ..> IConsultationService
VideoCallController ..> ILiveKitService
ExpertHub ..> ConsultationPingRequest
...
ConsultationLifecycleBackgroundService ..> IBookingService
ConsultationLifecycleBackgroundService ..> IConsultationPaymentService
ConsultationBooking ..> Consultation
ConsultationPingRequest ..> Consultation
ChatMessage ..> Consultation
Transaction ..> ConsultationBooking
Transaction ..> ConsultationPingRequest
