...
4. Thêm mới (`FE-05`) — thông tin cần nhập:
   - Tên khoa học (Latin)
   - Tên tiếng Việt
   - Tên địa phương
...
5. Upload hình ảnh rắn (`FE-06`):
   - Tối thiểu 5 ảnh, nhiều góc độ, chất lượng cao
   - Gắn tag: Đầu, thân, đuôi, hoa văn
6. Phân loại theo khu vực (`FE-08`): chọn tỉnh & mức độ phổ biến.
7. Lưu vào database.
8. Hệ thống tự động:
...
1. Admin mở Dashboard **"Giám sát Real-time"** (`FE-26`).
...
3. Admin có thể click vào điểm để xem chi tiết, theo dõi vị trí Rescuer & trạng thái nhiệm vụ (`FE-27`, `FE-29`).
4. Xem biểu đồ nhiệt (Heat Map) (`FE-28`) — phân bố sự cố theo thời gian & loài.
5. Nếu phát hiện bất thường: gửi cảnh báo cộng đồng (`FE-22`, `FE-24`) hoặc can thiệp vận hành (ưu tiên ca khẩn cấp).
...
2. Thiết lập phí dịch vụ (`FE-30`): phí cứu hộ, phí tư vấn, % hoa hồng nền tảng, % chia cho Rescuer, % quỹ bảo hiểm.
3. Xem báo cáo doanh thu (`FE-31`, `FE-33`) theo ngày/tuần/tháng/năm, theo Rescuer/Expert.
4. Quản lý thanh toán (`FE-32`): danh sách giao dịch chờ, kiểm tra & xử lý giao dịch lỗi.
5. Xử lý tranh chấp (`FE-35`): kiểm tra lịch sử GPS / trạng thái thanh toán & đưa ra quyết định hoàn tiền hoặc từ chối.
6. Quản lý hoàn tiền (`FE-34`): duyệt yêu cầu, hoàn tiền trong 3–5 ngày làm việc & cập nhật trạng thái.
7. Xuất báo cáo tài chính định kỳ (`FE-33`) cho Ban Giám đốc.
